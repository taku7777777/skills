...
def validate_result_rubric_drift(root: Path, validation: Validation) -> None:
    """Warn when an archived rubric no longer matches the grader now in use.

    Scored results under quality/results/ are only reproducible against the rubric
    they were scored with. A criterion mismatch in ids, descriptions, or critical flags
    means the published numbers cannot be regenerated by scripts/eval_tasks.py, which
    must be disclosed rather than fixed by silently re-scoring. This is a warning,
    never an error: a disclosed drift is a legitimate state for an archived run.
    """
    graders = root / "quality" / "graders"
    for archived_path in sorted((root / "quality" / "results").rglob("rubric.json")):
        archived = load_json(archived_path, validation)
        if not isinstance(archived, dict):
            continue
        case_id = archived.get("case_id")
        if not isinstance(case_id, str):
            validation.warn(f"{archived_path}: archived rubric has no usable case_id")
            continue
        current_path = graders / f"{case_id}.json"
        if not current_path.is_file():
            validation.warn(f"{archived_path}: no current grader for case {case_id!r}")
            continue
        current = load_json(current_path, validation)
        if not isinstance(current, dict):
            continue

        def criteria_by_id(rubric: dict[str, object]) -> dict[str, dict[str, object]]:
            criteria = rubric.get("criteria")
        ...
        archived_criteria = criteria_by_id(archived)
        current_criteria = criteria_by_id(current)
        archived_ids = set(archived_criteria)
        current_ids = set(current_criteria)
        common_ids = archived_ids & current_ids
        descriptions_changed = sorted(
            criterion_id
            for criterion_id in common_ids
            if archived_criteria[criterion_id].get("description")
            != current_criteria[criterion_id].get("description")
        )
        critical_flags_changed = sorted(
            criterion_id
            for criterion_id in common_ids
            if archived_criteria[criterion_id].get("critical")
            != current_criteria[criterion_id].get("critical")
        )
        drift_details: list[str] = []
        if archived_ids != current_ids:
            added = sorted(current_ids - archived_ids)
            removed = sorted(archived_ids - current_ids)
            drift_details.append(f"criterion ids added {added}, removed {removed}")
        if descriptions_changed:
            drift_details.append(f"criterion descriptions changed {descriptions_changed}")
        if critical_flags_changed:
            drift_details.append(f"criterion critical flags changed {critical_flags_changed}")
        if drift_details:
            validation.warn(
                f"{archived_path}: rubric drifted from {current_path} "
                f"({'; '.join(drift_details)}); the scored results in this directory "
                f"are not reproducible with the current grader and must say so in run.json"
            )
...
def main() -> int:
    ...
    validate_result_rubric_drift(root, validation)
    ...
    fully_executed = sum(
        1
        for skill_dir in skill_dirs
        if (skill_dir / "evals.md").is_file()
        and "(未実施)" not in (skill_dir / "evals.md").read_text(encoding="utf-8")
    )
    completed_cases = {path.parent.name for path in root.glob("quality/results/*/*/summary.json")}
    print(
        f"skills={len(skill_dirs)} errors={len(validation.errors)} "
        f"warnings={len(validation.warnings)} strict={args.strict}"
    )
    print(
        f"coverage: skills_without_pending_evaluations={fully_executed}/{len(skill_dirs)} "
        f"completed_task_cases={len(completed_cases)}"
    )
